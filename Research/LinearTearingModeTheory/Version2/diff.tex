...
%DIF DEL Paper1.tex   Wed Jan 26 11:29:21 2022
%DIF ADD Paper2.tex   Wed Jan 26 11:29:30 2022
\usepackage {amsmath}
\pdfoutput = 1 
\usepackage {graphicx}
...
necessitating a two-fluid treatment.\cite{ara} Moreover, resistive-MHD does not take  the important {\em ion sound radius\DIFdelbegin \DIFdel{lengthscale}\DIFdelend } \DIFaddbegin \DIFadd{lengthscale}\DIFaddend , below which electron and ion dynamics are further decoupled, 
...
plasma response model is introduced in Sect.~\ref{sfour}. This model is a slightly improved version of the model derived in Ref.~\onlinecite{fw} that takes the thermal force into account. \DIFaddbegin \DIFadd{(The thermal force merely gives rise to an additional term  in the perpendicular electron fluid velocity that scales as the electron temperature gradient.) }\DIFaddend The model is converted into a set of linear layer equations in
...
of a resonant layer to an externally generated\DIFaddbegin \DIFadd{, }\DIFaddend resonant magnetic perturbation in a large aspect-ratio tokamak plasma. In Sect.~\ref{stab}, we reuse the analysis of Sect.~\ref{linear} to map out all
...
in Sect.~\ref{error}, we employ the results of Sect.~\ref{linear} to determine the scaling of the error-field penetration threshold with machine parameters
 in large aspect-ratio \DIFdelbegin \DIFdel{, ohmically heated, }\DIFdelend tokamak plasmas. 
...
\begin{align}
\frac{\partial\psi}{\partial\hat{t}}&= [\phi,\psi] -\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdel{\frac{\tau}{1+\tau}}%DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \DIFadd{\tau_e\,}\DIFaddend (1+\lambda_e)\,[N,\psi]
+\hat{\eta}_\parallel\,J + \hat{E}_\parallel,\\[0.5ex]
\frac{\partial \hat{\nabla}^2\phi}{\partial \hat{t}}&= [\phi,\hat{\nabla}^2\phi] - \DIFdelbegin \DIFdel{\frac{1}{2\,(1+\tau)}}\DIFdelend \DIFaddbegin \DIFadd{\frac{\tau_i}{2}}\DIFaddend \left(\hat{\nabla}^2[\phi,N] + [\hat{\nabla}^2\phi,N] + [\hat{\nabla}^2 N,\phi]\right) + [J,\psi] \nonumber\\[0.5ex]&\phantom{=}+\hat{\chi}_\varphi  \,\hat{\nabla}^4\!\left(\phi + \DIFdelbegin \DIFdel{\frac{N}{1+\tau}}\DIFdelend \DIFaddbegin \DIFadd{\tau_i\,N}\DIFaddend \right), \\[0.5ex]
...
\end{align}
Here, $[A,B]\equiv \hat{\nabla} A\times \hat{\nabla} B\cdot {\bf n}$, \DIFaddbegin \DIFadd{$\tau_e=[\tau/(1+\tau)]$, $\tau_i=1/(1+\tau)$, }\DIFaddend $\hat{t} = t/(r_s/V_A)$, $\hat{\eta}_{\parallel,\perp} = \eta_{\parallel,\perp}/(\mu_0\,r_s\,V_A)$, $\hat{E}_\parallel = E_\parallel/(B_z\,V_A)$, 
...
$\lambda_e=0.71\,\eta_e/(1+\eta_e)$ \DIFaddbegin \DIFadd{originates from the thermal force}\DIFaddend . Finally,\cite{fw}
\begin{equation}
\hat{D}_\perp \equiv \frac{D_{\perp}}{r_s\,V_A}= c_\beta^{\,2}\,\hat{\eta}_\perp +  \frac{2}{3}\left(1-c_\beta^{\,2}\right)\!\left(\DIFdelbegin \DIFdel{\frac{\eta_e}{1+\eta_e}\,\frac{\tau}{1+\tau}}\DIFdelend \DIFaddbegin \DIFadd{\frac{\eta_e\,\tau_e}{1+\eta_e}}\DIFaddend \,\hat{\chi}_{\perp\,e} + \DIFdelbegin \DIFdel{\frac{\eta_i}{1+\eta_i}\,\frac{1}{1+\tau}}\DIFdelend \DIFaddbegin \DIFadd{\frac{\eta_i\,\tau_i}{1+\eta_i}}\DIFaddend \,\hat{\chi}_{\perp\,i}\right),
\end{equation}
...
correct in the present paper. \DIFaddbegin \DIFadd{To be more exact, given that $c_\beta\ll 1$ in conventional tokamak plasmas, and
...
N(\hat{x},\zeta,\hat{t}) &= -\hat{V}_\ast\,\hat{x}+\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdel{\frac{1+\tau}{\tau}}%DIFDELCMD < \right)%%%
\DIFdel{\!}\DIFdelend \DIFaddbegin \DIFadd{\tau_e\,}\DIFaddend \tilde{N}(\hat{x})\,{\rm e}^{\,{\rm i}\,(\zeta-\hat{\omega}\,\hat{t})},\\[0.5ex]
V(\hat{x},\zeta,\hat{t}) &= \hat{V}_\parallel +\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdel{\frac{1+\tau}{\tau}}%DIFDELCMD < \right)%%%
...
-{\rm i}\,(\omega-\omega_E)\,\tau_H\,\tilde{N} &= -{\rm i}\,\omega_{\ast\,e}\,\tau_H\,\tilde{\phi}-{\rm i}\,c_\beta^{\,2}\,\hat{x}\,\tilde{V} - {\rm i}\DIFdelbegin \DIFdel{\!}%DIFDELCMD < \left(%%%
\DIFdel{\frac{\tau}{1+\tau}}%DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \DIFadd{\,\tau_e\,}\DIFaddend \hat{d}_\beta^{\,2}\,\hat{x}\,\hat{\nabla}^2\tilde{\psi}\nonumber\\[0.5ex]
...
\DIFdelbegin \DIFdel{$\omega_{\ast\,e} = -[\tau/(1+\tau)](m/r_s)V_\ast(r_s)$
}\DIFdelend \DIFaddbegin \DIFadd{$\omega_{\ast\,e} = -\tau_e\,(m/r_s)V_\ast(r_s)$
}\DIFaddend the 
   electron diamagnetic
frequency,
\DIFdelbegin \DIFdel{$\omega_{\ast\,i} =[1/(1+\tau)](m/r_s)V_\ast(r_s)$
 }\DIFdelend \DIFaddbegin \DIFadd{$\omega_{\ast\,i} =\tau_i\,(m/r_s)V_\ast(r_s)$
 }\DIFaddend the  ion diamagnetic
frequency, 
$S=\tau_R/\tau_H$
...
cases $D<1$ and $D>1$, respectively. \DIFaddbegin \DIFadd{Note that
}\begin{align}
\DIFadd{Q}&\DIFadd{\simeq 1.02\times 10^2\left[\frac{r_s^{2/3}({\rm m})\,L_s^{\,2/3}({\rm m})\,T_0^{\,1/2}({\rm keV})\,n_0^{\,1/2}(10^{19}\,{\rm m}^{-3})\,\omega({\rm krad/s})}{m^{\,2/3}\,B_z^{\,2/3}({\rm T})}\right],\label{e73}}\\[0.5ex]
\DIFadd{P}&\DIFadd{\simeq 5.04\times 10^1\left[\chi_\perp({\rm m^2/s})\,T_0^{\,3/2}({\rm keV})\right],}\\[0.5ex]
\DIFadd{D}&\DIFadd{\simeq 4.15\left[\frac{\tau_e^{\,1/2}\,m^{\,1/3}\,T_0({\rm keV})}{r_s^{\,1/2}({\rm m})\,L_s^{\,1/2}({\rm m})\,B_z^{\,1/6}({\rm T})\,n_0^{\,1/6}(10^{19}\,{\rm m}^{-3})}\right],
}\end{align}
\DIFadd{where $T_0$ and $\chi_\perp$ are the electron/ion temperatures and the particle/energy diffusivities at the resonant
surface.
 }\DIFaddend It can be seen, by comparison with Figs.~1--3 in Ref.~\onlinecite{cole}, that
...
where 
\begin{align}
A(p) &= (1+\lambda_e)\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdel{\frac{\tau}{1+\tau}}%DIFDELCMD < \right)%%%
...
-{\rm i}\left(1+\DIFdelbegin \DIFdel{\frac{\lambda_e\,\tau}{1+\tau}}\DIFdelend \DIFaddbegin \DIFadd{\lambda_e\,\tau_e}\DIFaddend \right)Q_\ast\,(P_\varphi+P_\perp)\,p^2\DIFdelbegin %DIFDELCMD < \nonumber\\[0.5ex]
%DIFDELCMD < &\phantom{=} %%%
\DIFdelend + P_\varphi\,P_\perp\,p^4,\\[0.5ex]
\tilde{B}(p)&= P_\perp - {\rm i}\,(1+\lambda_e)\left(1+\DIFdelbegin \DIFdel{\frac{\lambda_e\,\tau}{1+\tau}}\DIFdelend \DIFaddbegin \DIFadd{\lambda_e\,\tau_e}\DIFaddend \right)Q_\ast\,D^2 +\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdel{\frac{1+\tau}{\tau}}%DIFDELCMD < \right)%%%
...
Hence, 
\begin{equation}
\hat{\mit\Delta} = \pi\,\hat{\gamma}\left[ \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdel{\frac{\tau}{1+\tau}}%DIFDELCMD < \right)%%%
...
\begin{equation}
C = {\rm i}\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdel{\frac{\tau}{1+\tau}}%DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \DIFadd{\,\tau_e\,}\DIFaddend \frac{Q_\ast}{D^2}.
\end{equation} 
Hence, 
\begin{equation}
\hat{\mit\Delta} = {\rm e}^{{\rm i}\,\pi/4}\,\pi\,\hat{\gamma}\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdel{\frac{\tau}{1+\tau}}%DIFDELCMD < \right)%%%
...
sound radius $D$).
 ...
 There are four tearing-mode growth-rate regimes---the resistive-inertial, the viscous-resistive, the semi-collisional, and the diffusive-resistive---and their extents in $Q_\ast$-$P$ space are illustrated in Fig.~\ref{f3}. \DIFaddbegin \DIFadd{The value of $Q_\ast$
 can be estimated from Eq.~(\ref{e73}), with $\omega$ replaced by $\omega_\ast$. 
}\DIFaddend Note that Fig.~\ref{f3} differs somewhat from Figs.~\ref{f1} and \ref{f2} because in the latter two
...
perpendicular transport. \DIFaddbegin \DIFadd{To be more exact, the spurious resonances at  $\omega_r=\omega_E$ and $\omega_r=\omega_E+\omega_{\ast\,i}$  originate from the fact that, in the absence of finite
...
containing locked magnetic island chains often terminate in disruptions\DIFdelbegin \DIFdel{\,\mbox{%DIFAUXCMD
\cite{wes1} }\hspace{0pt}%DIFAUXCMD
(usually triggered when the
external heating is turned on).}\DIFdelend \DIFaddbegin \DIFadd{.\mbox{%DIFAUXCMD
\cite{wes1} }\hspace{0pt}%DIFAUXCMD
...
Making use of the identities \DIFaddbegin \DIFadd{(which have been verified numerically)
}\DIFaddend \begin{align}\label{e417a}
...
\begin{align}
\rho_\ast &= \frac{T_0^{1/2}\,m_i^{1/2}}{e\,B_z\,L_s}\DIFaddbegin \DIFadd{= 3.23\times 10^{-3}\,\frac{T_0^{\,1/2}({\rm keV})}{B_z({\rm T})\,L_s({\rm m})}}\DIFaddend ,\label{e428}\\[0.5ex]
\nu_\ast &= \frac{L_s\,n_0\,e^2\,\eta_\parallel}{m_e^{1/2}\,T_0^{1/2}}\DIFaddbegin \DIFadd{= 5.30\times 10^{-4}\,\frac{L_s({\rm m})\,n_0(10^{19}\,{\rm m}^{-3})}{T_0^{\,2}({\rm keV})}}\DIFaddend ,\\[0.5ex]
\beta_\ast&=\frac{\mu_0\,n_0\,T_0}{B_z^{\,2}}\DIFaddbegin \DIFadd{= 2.01\times 10^{-3}\,\frac{n_0(10^{19}\,{\rm m}^{-3})\,T_0({\rm keV})}{B_z^{\,2}({\rm T})}}\DIFaddend ,\label{e430}
\end{align}
...
In \DIFdelbegin \DIFdel{an ohmically-heated }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend tokamak
plasma, the energy confinement timescale, $\tau_E$, satisfies the constraint
\begin{equation}
\frac{n_0\,T_0}{\tau_E} = \eta_\parallel\left(\frac{B_z}{\mu_0\,L_s}\right)^2\DIFaddbegin \DIFadd{\,{\cal P}}\DIFaddend ,
\end{equation}
which is obtained by equating the energy loss rate to the \DIFaddbegin \DIFadd{heating rate. Here, ${\cal P}$ is the ratio of the total heating rate to the }\DIFaddend ohmic heating rate. \DIFaddbegin \DIFadd{(Thus, ${\cal P}=1$ in an ohmically heated tokamak.)
 }\DIFaddend Let us assume that the momentum confinement
time, $\tau_\varphi$, and the particle/energy confinement time, $\tau_\perp$, are both equal to $\tau_E$, as is generally
(approximately) the case in  \DIFdelbegin \DIFdel{ohmically-heated }\DIFdelend tokamak plasmas.\cite{iter} 
...
\DIFaddbegin \subsection{\DIFadd{Ohmic Plasmas}}
\DIFaddend In order to make further progress, we need to adopt a specific scaling model for the energy confinement timescale, $\tau_E$. \DIFaddbegin \DIFadd{Consider ohmic plasmas for which ${\cal P}=1$. }\DIFaddend Let us adopt
...
\end{equation}
(The modification is to ensure that the scaling law is consonant with the Connor-Taylor constraints\DIFaddbegin \DIFadd{, which
require that ${\mit\Omega}_i\,\tau_E\propto n^{\alpha_n}\,B^{\,\alpha_B}\,R^{\,2\,\alpha_n+(5/4)\,\alpha_B}$}\DIFaddend .\cite{connor})
It follows that
\begin{align}
...
\left(\frac{b_{r\,{\rm crit}}}{B_z}\right)_{{\rm VR}-\varphi} &\propto n\DIFdelbegin \DIFdel{^{0}}\DIFdelend \DIFaddbegin \DIFadd{^{\,0}}\DIFaddend \,B^{-13/15}\,R^{-13/12}\DIFaddbegin \DIFadd{= n^{\,0.00}\,B^{-0.87}\,R^{-1.08}}\DIFaddend ,\\[0.5ex]
\left(\frac{b_{r\,{\rm crit}}}{B_z}\right)_{{\rm VR}-\theta} &\propto n\DIFdelbegin \DIFdel{^{1/2}}\DIFdelend \DIFaddbegin \DIFadd{^{\,1/2}}\DIFaddend \,B^{-7/6}\,R^{-11/24}\DIFaddbegin \DIFadd{= n^{\,0.50}\,B^{-1.17}
\,R^{-0.46}}\DIFaddend ,\\[0.5ex]
\left(\frac{b_{r\,{\rm crit}}}{B_z}\right)_{{\rm DR}-\theta} &\propto n\DIFdelbegin \DIFdel{^{1/2}}\DIFdelend \DIFaddbegin \DIFadd{^{\,1/2}}\DIFaddend \,B^{-23/20}\,R^{-7/16}\DIFaddbegin \DIFadd{=n^{\,0.50}\,B^{-1.15}\,R^{-0.44}}\DIFaddend ,\\[0.5ex]
\left(\frac{b_{r\,{\rm crit}}}{B_z}\right)_{{\rm DR}-\varphi} &\propto n\DIFdelbegin \DIFdel{^{0}}\DIFdelend \DIFaddbegin \DIFadd{^{\,0}}\DIFaddend \,B^{-17/20}\,R^{-17/16}\DIFaddbegin \DIFadd{= n^{\,0.00}\,B^{-0.85}\,R^{-1.06}}\DIFaddend .\label{e458}
\end{align}
...
\DIFaddbegin \subsection{\DIFadd{L/H-Mode Plasmas}}
\DIFadd{For externally heated (i.e., ${\cal P}>1$) L/H-mode plasmas, we adopt the ITER89-P scaling law,\mbox{%DIFAUXCMD
\cite{yush}
}\hspace{0pt}%DIFAUXCMD
which yields
}\begin{equation}
\DIFadd{T\propto n^{-18/35}\,B^{\,41/35}\,R^{\,27/35}\,{\cal P}^{\,2/7}.
}\end{equation}
\DIFadd{(Note that we are assuming that the plasma rotation remains diamagnetic, which means that we are not taking into
...
Hence, we deduce that 
}\begin{align}\DIFadd{\label{e141}
\left(\frac{b_{r\,{\rm crit}}}{B_z}\right)_{{\rm VR}-\varphi} }&\DIFadd{\propto n^{-3/35}\,B^{-169/210}\,R^{-218/210}\,{\cal P}^{\,53/84}= n^{-0.09}\,B^{-0.80}\,R^{-1.04}\,{\cal P}^{\,0.63},}\\[0.5ex]
\DIFadd{\left(\frac{b_{r\,{\rm crit}}}{B_z}\right)_{{\rm VR}-\theta} }&\DIFadd{\propto n^{\,2/7}\,B^{-85/84}\,R^{-29/84}\,{\cal P}^{\,19/42}= n^{\,0.29}\,B^{-1.01}
\,R^{-0.35}\,{\cal P}^{\,0.45},}\\[0.5ex]
\DIFadd{\left(\frac{b_{r\,{\rm crit}}}{B_z}\right)_{{\rm DR}-\theta} }&\DIFadd{\propto n^{\,61/140}\,B^{-309/280}\,R^{-113/280}\,{\cal P}^{\,23/56}=n^{\,0.44}\,B^{-1.10}\,R^{-0.40}\,{\cal P}^{\,0.41},}\\[0.5ex]
\DIFadd{\left(\frac{b_{r\,{\rm crit}}}{B_z}\right)_{{\rm DR}-\varphi} }&\DIFadd{\propto n^{\,9/140}\,B^{-251/280}\,R^{-307/280}\,{\cal P}^{\,33/56}= n^{\,0.06}\,B^{-0.90}\,R^{-1.09}\,{\cal P}^{\,0.59}.\label{e144}
}\end{align}
\DIFadd{It can be seen, by comparison with Eqs.~(\ref{e455})--(\ref{e458}), that the ITER89-P scaling law leads to a
...
\subsection{\DIFadd{Comparison with Experimental Data}}
\DIFadd{The most up to date and comprehensive study of the empirical scaling of the error-field penetration threshold
with machine parameters in tokamak plasmas is that of Ref.~}\onlinecite{logan}\DIFadd{. For $n=1$ modes in L/H-mode plasmas,
it is found that
}\begin{equation}
\DIFadd{\frac{b_{r\,{\rm crit}}}{B_z}\propto n^{\,0.58\pm 0.06}\,B^{-1.13\pm 0.07}\,R^{+0.10\pm0.07}.
}\end{equation}
\DIFadd{It can be seen, by comparison with Eqs.~(\ref{e141})--(\ref{e144}), that the experimental density and magnetic field-strength scalings of the penetration threshold are reasonably compatible with those predicted in the theoretical diffusive-resistive-poloidal (DR-$\theta$) regime. On the other hand, the scaling with machine size is significantly different. All of the theoretical
error-field penetration regimes lead to a }{\em \DIFadd{negative}}\DIFadd{\/ scaling of the penetration threshold with machine size.
On the other hand, the experimental scaling features a }{\em \DIFadd{positive}}\DIFadd{\/ scaling with machine size. This discrepancy has
...
large aspect-ratio \DIFdelbegin \DIFdel{, ohmically heated, }\DIFdelend tokamak plasmas. We find that \DIFdelbegin \DIFdel{if the change in the ${\bf E}\times {\bf B}$ frequency at the the resonant surface that permits the
driven mode to lock to the }\DIFdelend \DIFaddbegin \DIFadd{one particular }\DIFaddend error-field \DIFdelbegin \DIFdel{is predominately caused by a  shift in the  }%DIFDELCMD < {\em %%%
\DIFdel{poloidal}%DIFDELCMD < }%%%
\DIFdel{\/ ion fluid velocity (which is entirely possible, even in the presence of strong poloidal flow damping)
then the threshold exhibits an $n^{1/2}$ scaling with particle
number density . Such a scaling is consistent with some experimental observations}\DIFdelend \DIFaddbegin \DIFadd{penetration regime (i.e., the diffusive-resistive-poloidal)
...
significant consequences for ITER, which is much larger than present-day tokamaks}\DIFaddend .
...
\bibitem{rf2012} R.~Fitzpatrick, Plasma Phys.\ Control.\ Fusion {\bf 54}, 094002 (2012\DIFaddbegin \DIFadd{).
}

\bibitem{yush} \DIFadd{P.N.~Yushmanov, T.~Takizuka, K.S.~Reidel, O.J.W.F.~Kardaun, J.G.~Cordey, S.M.~Kaye,
and D.E.~Post, Nucl.\ Fusion }{\bf \DIFadd{30}}\DIFadd{, 1999 (1990).
}

\bibitem{logan} \DIFadd{N.C.~Logan, J.-K.~Park, Q.~Hu, C.~Paz-Soldan, T.~Markovic, H.~Wang,
Y.~In, L.~Piron, P.~Piovesan, C.E.~Myers, et al., Nucl.\ Fusion }{\bf \DIFadd{60}}\DIFadd{, 086010 (2020}\DIFaddend ).
