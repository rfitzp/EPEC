...
\section{Useful Identities}\label{s2}
...
\end{align}
Normally, all three components are zero. However, at $k$th rational 
surface $\psi_k$, $\psi_{j\neq k}$, and $\chi_{j\neq k}$ are continuous, whereas $\chi_k$ is discontinuous. Hence,
\begin{align}
...
Follows that
\begin{align}
\delta T_{\theta\,k} &= -\frac{2\pi^{\,2}\,R_0\,m_k}{\mu_0}\,{\rm Im}\left({\mit\Psi}_k^{\,\ast}\,{\mit\Delta\Psi}_k\right),\\[0.5ex]
\delta T_{\phi\,k} &= \frac{2\pi^{\,2}\,R_0\,n}{\mu_0}\,{\rm Im}\left({\mit\Psi}_k^{\,\ast}\,{\mit\Delta\Psi}_k\right).
\end{align}
...
so
\begin{equation}
\delta A_\phi(r,\theta,0)= \frac{1}{4\pi}\int_0^\infty
\oint\oint \frac{R\,R'\,\mu_0\,\delta J^{\,\phi}(r',\theta',0)\,{\rm e}^{-{\rm i}\,n\,\phi'}\,\cos\phi'\,{\cal J}'\,dr'\,d\theta'\,d\phi'}{\left[R^{\,2}+R'^{\,2} +(Z-Z')^{\,2} -2\,R\,R'\,\cos\phi'\right]^{\,1/2}},
\end{equation}
which can be written
\begin{equation}
\delta A_\phi(r,\theta,0)= \frac{1}{4\pi}\int_0^\infty
\oint R\,R'\,\mu_0\,\delta J^{\,\phi}(r',\theta',0)\,G(r,\theta;r',\theta')\,{\cal J}'\,dr'\,d\theta',
\end{equation}
where
\begin{equation}
G(r,\theta;r',\theta') = \oint \frac{\cos \phi'\,\cos(n\,\phi')\,d\phi'}{\left[R^{\,2}+R'^{\,2} +(Z-Z')^{\,2} -2\,R\,R'\,\cos\phi'\right]^{\,1/2}},
\end{equation}
or
\begin{equation}
G(r,\theta;r',\theta') = \frac{1}{2}\oint \frac{\left(\cos[(n-1)\,\phi']+\cos[(n+1)\,\phi']\right)\,d\phi'}{\left[R^{\,2}+R'^{\,2} +(Z-Z')^{\,2} -2\,R\,R'\,\cos\phi'\right]^{\,1/2}}.
\end{equation}
...
It follows that
\begin{align}
G(r,\theta;r',\theta')&=\pi\left[\frac{\cosh\eta}{R^{\,2}+R'^{\,2}+(Z-Z')^{\,2}}\right]^{1/2}\nonumber\\[0.5ex]
&
...
so
\begin{align}
G(r,\theta;r',\theta')&= \frac{(-1)^{\,n+1}\,\pi^{\,2}}{\Gamma(1/2)\,\Gamma(n+1/2)}
...
\begin{equation}
F_{kk'} = \oint\oint 
{\cal G}(r_k,\theta;r_{k'},\theta')\,{\rm e}^{-{\rm i}\,(m_k\,\theta-m_{k'}\,\theta')}\,\frac{d\theta}{2\pi}\,\frac{d\theta'}{2\pi},
\end{equation}
and
\begin{align}
{\cal G}(r,\theta;r',\theta')&= \frac{(-1)^{\,n}\,\pi^{\,2}\,R\,R'/R_0}{2\,\Gamma(1/2)\,\Gamma(n+1/2)}
\left[\frac{\cosh\eta}{R^{\,2}+R'^{\,2}+(Z-Z')^{\,2}}\right]^{1/2}\nonumber\\[0.5ex]
&\times\left[(n-1/2)\,P_{-1/2}^{\,n-1}(\cosh\eta)+
\frac{P_{-1/2}^{\,n+1}(\cosh\eta)}{n+1/2}\right].
\end{align}
Note that
\begin{equation}
{\cal G} (r',\theta';r,\theta) = {\cal G}(r,\theta;r',\theta'),
\end{equation}
which implies that
\begin{equation}
F_{k'k} = F_{kk'}^{\,\ast}.
...
It follows that
\begin{equation}
{\mit\Delta\Psi}_k =\sum_{k'} E_{kk'}\,{\mit\Psi}_{k'}+ |E_{kk}|\,\chi_k
\end{equation}
...
m_k\,\theta - n\,\phi)}
\end{equation}
in the vicinity of the $k$th rational surface. It follows that
\begin{equation}
...
\end{equation}
It follows that $W_k$ is the full radial island width (which is constant in $r$).

\section{Flux Coordinate System}
...
Note that
\begin{equation}
\frac{d\theta}{d\zeta} = \frac{g}{q\,|\psi_c|}\,\frac{F}{R}.
\end{equation}
Hence, [from~(\ref{e140})]
\begin{equation}
\langle A\rangle = \left.\oint R^{\,2}\,A\,d\theta\right/\oint R^{\,2}\,d\theta = \left.\oint F\,R\,A\,d\zeta\right/\oint F\,R\,d\zeta.
\end{equation}
In particular, because
\begin{equation}
B^{\,2} = \frac{\psi_c^{\,2}\,|\nabla{\mit\Psi}|^{\,2}}{R^{\,2}} + \frac{g^{\,2}}{R^{\,2}}
\end{equation}
it follows that
\begin{equation}
\langle B^{\,2}\rangle = \left.\oint \frac{F}{R}\left[\psi_c^{\,2}\left({\mit\Psi}_R^{\,2}+ {\mit\Psi}_Z^{\,2}\right) + g^{\,2}\right]d\zeta\right/\oint F\,R\,d\zeta.
\end{equation}
...
$k$th rational surface then
\begin{equation}
\overline{W}_k = \frac{W_k}{R_0}\,\frac{d{\mit\Psi}_N}{dr}.
\end{equation}

\section{GPEC Coupling}
We have
\begin{align}
...
which implies that
\begin{align}
{\mit\Delta}_j &={\rm i}\left(\frac{R_0}{r_j}\right)^2 \frac{m_j\,a_{jj}}{g_j}\,\frac{{\mit\Delta\Psi}_j}{B_0\,R_0},\\[0.5ex]
...
\\[0.5ex]
\langle \sin(k\,{\mit\Theta}) \,({\bf b}\cdot\nabla \ln B)\rangle &= \gamma\,\frac{I_{4,k}}{I_1},\\[0.5ex]
\left\langle \sin(k\,{\mit\Theta}) \,\frac{({\bf b}\cdot\nabla \ln B)}{B}\right\rangle &= \gamma\,\frac{I_{5,k}}{I_1}.
\end{align}
Hence,
\begin{align}
L_c &= \frac{1}{|\gamma|}\,\frac{I_2^{\,2}}{I_1^{\,2}\,I_3}\sum_{k>0}
\frac{2}{k}\,I_{5,k}\,I_{6,k},\\[0.5ex]
