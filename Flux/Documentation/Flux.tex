...
\begin{equation}
X_k^{\,2}+  A_k^{(2)}\,X_k^{\,3}+ A_k^{(3)}\,X_k^{\,4} = \frac{\overline{W}_k^{\,2}}{4},
\end{equation}
where
\begin{equation}
...
A_k^{(2)}& =  \frac{1}{3}\,\frac{q_k''}{q_k'},\\[0.5ex]
A_k^{(3)}&= 
\frac{1}{12}\,
\frac{q_k'''}{q_k'}.
\end{align}
...
\paragraph{Scheme 2:}
Alternatively, we can say that $X_{k\,\pm}$ satisfy 
\begin{equation}
F_k(X_k) = \frac{|{\mit\Psi}_k|}{R_0\,B_0},
\end{equation}
where
\begin{equation}
\frac{dF_k}{dX_k} = \frac{|\psi_c|}{2}\left[\frac{q({\mit\Psi}_{N\,k}+X_k)}{q_k}-1\right].
\end{equation}

\paragraph{Scheme 3:}
Alternatively, can say that $X_{k\pm} = \pm{\mit\Delta}_{k\pm}\,x_{k\pm}$,
where
\begin{align}
{\mit\Delta}_{k+} &= {\mit\Psi}_{N\,k+1}-{\mit\Psi}_{N\,k},\\[0.5ex]
{\mit\Delta}_{k-} &= {\mit\Psi}_{N\,k}-{\mit\Psi}_{N\,k-1},\\[0.5ex]
