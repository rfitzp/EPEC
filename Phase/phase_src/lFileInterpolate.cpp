...
// ###############################
void Phase::lFileInterpolate (char* lFile1, double time1, char* lFile2, double time2, char* lFile3, double time3, char* lFile, double time)
{
  char line1[500], line2[500], line3[500], line4[500], line5[500],
    linea[500], lineb[500], linec[500], lineaa[500], linebb[500], linecc[500], line7[500], line8[500];
  ...
  fgets (line1, 500, file);
  fgets (line2, 500, file);
  fgets (line3, 500, file);
  fgets (line4, 500, file);
  fgets (line5, 500, file);
  fgets (linea, 500, file);
  fgets (line7, 500, file);
  fgets (line8, 500, file);
  strcpy (lineaa, linea);
...
void Phase::lFileInterp (vector<string> lFileName, vector<double> lFileTime, vector<int> lFileNtor, vector<int> lFileMmin, vector<int> lFileMmax, int lFileNumber,
			 double time, int &Ntor, int& Mmin, int& Mmax)
{
  ...
  if (time < lFileTime[0])
    ...
    {
      index = lFileNumber - 3;
      _time = lFileTime[lFileNumber-1];
      Ntor  = lFileNtor[lFileNumber-1];
      Mmin  = lFileMmin[lFileNumber-1];
      Mmax  = lFileMmax[lFileNumber-1];
    }
  else
    {
	...
	if (time >= lFileTime[i] && time < lFileTime[i+1])
	  {
	    ...
	    if (index > lFileNumber-3)
	      index = lFileNumber - 3;

	    if (lFileMmax[i+1] > lFileMmax[i])
	      {
		Ntor = lFileNtor[i];
		Mmin = lFileMmin[i];
		Mmax = lFileMmax[i];
  ...
  lFileInterpolate (file1, lFileTime[index], file2, lFileTime[index+1], file3, lFileTime[index+2], lFile, _time);
}
