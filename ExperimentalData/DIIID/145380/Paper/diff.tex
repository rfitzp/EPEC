...
%DIF DEL paper.tex    Wed Aug 12 14:53:44 2020
%DIF ADD paper1.tex   Sat Sep 19 09:57:17 2020
\usepackage {amsmath}
\pdfoutput = 1 
\usepackage {graphicx}
...
\title {\DIFdelbegin \DIFdel{Modelling }\DIFdelend \DIFaddbegin \DIFadd{Modeling }\DIFaddend $q_{95}$ Windows for the Suppression of Edge Localized Modes by Resonant Magnetic Perturbations in the DIII-D Tokamak}
...
determined by the ion flows at the corresponding \DIFdelbegin \DIFdel{rational }\DIFdelend \DIFaddbegin \DIFadd{resonant }\DIFaddend surfaces, which is the prediction of nonlinear \DIFaddbegin \DIFadd{tearing mode }\DIFaddend theory.
In the second case, the natural frequencies are 
determined by the \DIFdelbegin \DIFdel{guiding-center flows at the rational }\DIFdelend \DIFaddbegin \DIFadd{local ${\bf E}\times {\bf B}$ velocities at the resonant }\DIFaddend surfaces.
 In the third case, the natural frequencies   are 
determined by the electron flows at the  \DIFdelbegin \DIFdel{rational }\DIFdelend \DIFaddbegin \DIFadd{resonant }\DIFaddend surfaces, which is the prediction of linear \DIFaddbegin \DIFadd{tearing mode }\DIFaddend theory.
The second case gives  the best agreement between the simulations and the  experimental observations.    The \DIFdelbegin \DIFdel{third case gives by far the worst agreement between the simulations 
and the  observations. The first case }\DIFdelend \DIFaddbegin \DIFadd{first and third cases }\DIFaddend only leads to partial agreement between  the  simulations and the  observations. \DIFdelbegin \DIFdel{However, this }\DIFdelend \DIFaddbegin \DIFadd{In the first case, the }\DIFaddend lack of complete agreement
may be a consequence of using an inaccurate assumption for the neutral particle distribution in the pedestal.  \DIFaddbegin \DIFadd{In the third case, the lack of complete agreement is probably
...
tokamak discharge), to an H-mode tokamak discharge is observed to give rise to  two distinct phenomena.\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{schmitz, lanctot,paz1,d158115,paz} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{schmitz, lanctot,paz1,d158115,lyons,paz} }\hspace{0pt}%DIFAUXCMD
...
studies. The \DIFdelbegin \DIFdel{primary }\DIFdelend \DIFaddbegin \DIFadd{first }\DIFaddend aim of this paper is to employ this
new model to try to account for the  $q_{95}$
ELM-suppression\DIFaddbegin \DIFadd{/mitigation }\DIFaddend windows  that are apparent when the edge safety-factor is slowly ramped in
a particular DIII-D discharge (\#145380) in which an $n=3$
RMP is used to control ELMs.\cite{d3d,d3d2} \DIFaddbegin \DIFadd{The second aim is to determine  which choice for the so-called ``natural frequency''  of tearing modes can best account for the   $q_{95}$ 
ELM-suppression/mitigation windows  observed in DIII-D discharge \#145380. 
}\DIFaddend 

\DIFaddbegin \section{\DIFadd{Natural Frequency}}
\subsection{\DIFadd{Introduction}}
\DIFaddend It is well-known that magnetic reconnection driven when a  (stable) tearing mode interacts with a static RMP that is resonant at a particular magnetic flux-surface in a tokamak plasma is
facilitated when the associated \DIFdelbegin \DIFdel{``natural frequency'' }\DIFdelend \DIFaddbegin \DIFadd{natural frequency, in the absence of the RMP,  }\DIFaddend is relatively small.\cite{rfa,rfb}  The natural frequency  of a  (stable) tearing mode is the
helical phase velocity that the mode would possess were it naturally unstable\DIFdelbegin \DIFdel{(in the absence of the RMP). }\DIFdelend \DIFaddbegin \DIFadd{.  (It should be noted that the maximum practical currents
...
phase velocities of such modes fall below about 5 krad/s. A typical phase velocity is 100 krad/s.) }\DIFaddend Driven magnetic reconnection leads to the
...
However, there is currently some \DIFdelbegin \DIFdel{uncertainty }\DIFdelend \DIFaddbegin \DIFadd{debate }\DIFaddend in the fusion community regarding the \DIFdelbegin \DIFdel{form of the natural frequency}\DIFdelend \DIFaddbegin \DIFadd{appropriate choice for the natural frequency. 
}

 \DIFadd{The natural frequency of a tearing mode 
...
surface. This is indeed the case in the linear regime.\mbox{%DIFAUXCMD
\cite{ara} }\hspace{0pt}%DIFAUXCMD
...
\DIFadd{The situation is very different in the nonlinear regime. 
...
Of course, it is the ion fluid which is more resistant because of its much greater perpendicular viscosity,\mbox{%DIFAUXCMD
\cite{nl1,wat} }\hspace{0pt}%DIFAUXCMD
as well as its much larger 
neoclassical stress tensor.\mbox{%DIFAUXCMD
\cite{nl2} }\hspace{0pt}%DIFAUXCMD
Hence, a nonlinear magnetic island chain is convected by the ion fluid in the vicinity of the resonant 
surface, because this choice of propagation speed minimizes the ion fluid velocity gradient across the separatrix.
}

\DIFadd{Finally, we could imagine that if the width of an island chain is neither much less than the linear layer width (which is the strict
...
\DIFadd{It is clear, from the preceding discussion,  that there are three main choices for the natural frequency of a given tearing mode in a tokamak plasma}\DIFaddend . 

\DIFdelbegin \DIFdel{According }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Case 1}}
\DIFadd{As we have already mentioned, according }\DIFaddend to nonlinear tearing mode theory, a tearing mode is essentially convected by
the local ion fluid at the \DIFdelbegin \DIFdel{rational surface(see the discussion in Sect.~\ref{natural}).}\DIFdelend \DIFaddbegin \DIFadd{resonant surface.}\DIFaddend \cite{nl1,nl2,nl3}
...
toroidal periods, respectively, of the helical magnetic island chain driven at the rational surface. Moreover, ${\mit\Psi}_t({\mit\Psi}_p)$ is the equilibrium toroidal \DIFaddbegin \DIFadd{(poloidal) }\DIFaddend magnetic flux (divided by $2\pi$).

\DIFaddbegin \subsection{\DIFadd{Case 2}}
\DIFaddend A second possibility is that a tearing mode is convected \DIFdelbegin \DIFdel{by the local guiding-center fluid at the rational surface(again,see the discussion in Sect.~\ref{natural}),\mbox{%DIFAUXCMD
\cite{heyn,paz1}
}\hspace{0pt}%DIFAUXCMD
in }\DIFdelend \DIFaddbegin \DIFadd{at the local ${\bf E}\times{\bf B}$ velocity at the resonant surface,\mbox{%DIFAUXCMD
\cite{heyn,lyons,paz1}
}\hspace{0pt}%DIFAUXCMD
in }\DIFaddend which case we would expect the natural frequency to take the form 
...
\DIFaddbegin \subsection{\DIFadd{Case 3}}
\DIFaddend Finally, according to
linear tearing mode theory, a tearing mode is essentially convected by
the local electron fluid at the resonant surface\DIFdelbegin \DIFdel{(again, see the discussion in Sect.~\ref{natural}).}\DIFdelend \DIFaddbegin \DIFadd{.}\DIFaddend \cite{hender,cole} Hence, if the response of an H-mode tokamak plasma to an applied RMP
...
\DIFdelbegin \DIFdel{The secondary aim of this paper is to determine which of the three aforementioned choices for the natural frequency can best account for the   $q_{95}$ 
ELM-suppression windows  observed in DIII-D discharge \#145380. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{Description of Theoretical Model}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{The }\DIFdelend \DIFaddbegin \section{\DIFadd{EPEC Modeling of DIII-D Discharge \#145380}}
\subsection{\DIFadd{EPEC Code}}
\DIFadd{The }\DIFaddend theoretical model of the response of a tokamak plasma to an externally applied RMP that is used in this
paper is described in detail in Ref.~\onlinecite{rf2}. The model employs a standard asymptotic matching approach.\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{fkr,am1,am2,am3}
}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{fkr,coppi,ruth,ara,pletz,am1,pletz1,tokuda,brennan,fura,am2,am3}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend According to this approach, the response of the plasma to the applied RMP is governed by a combination of flux-freezing and
perturbed force balance (this combination is often referred to as ``marginally-stable ideal-MHD"\DIFdelbegin \DIFdel{, which is a misnomer because MHD }%DIFDELCMD < {\em %%%
\DIFdel{per se}%DIFDELCMD < }%%%
\DIFdel{\/ plays no
role}\DIFdelend ) everywhere in the plasma apart from a number of relatively narrow (in the radial direction) regions in which the applied
...
driven at the resonant surfaces \DIFaddbegin \DIFadd{(in DIII-D discharge 158115,\mbox{%DIFAUXCMD
\cite{rf1} }\hspace{0pt}%DIFAUXCMD
which is similar to DIII-D discharge 145380) }\DIFaddend exceed the linear layer widths (which invalidates linear theory).
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{rf1}
}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{(This state of affairs can be expected to persist in future tokamaks, such as ITER, because such tokamaks will operate with much higher values of the
Lundquist number than present-day tokamaks, leading to a reduction in the linear layer widths.)
}\DIFaddend Thus, when employing the asymptotic matching approach, the equations of flux-freezing and perturbed force balance
...
A toroidal tokamak equilibrium exhibits two distinct types of response to an applied RMP.\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
the aforementioned scheme. The theoretical model \DIFdelbegin \DIFdel{used in this paper actually requires  plasma equilibria and }\DIFdelend \DIFaddbegin \DIFadd{implemented in the }{\tt \DIFadd{EPEC}} \DIFadd{code actually requires  }\DIFaddend profiles that are unaffected
by the applied RMP \DIFaddbegin \DIFadd{(to be more exact, it requires sufficient information to reconstruct  natural frequencies that are unmodified by the electromagnetic
torques that develop in the plasma in response to the RMP)}\DIFaddend . However, there is clear evidence that the  \DIFdelbegin \DIFdel{equilibria and the profiles  (in particular) }\DIFdelend \DIFaddbegin \DIFadd{profiles  }\DIFaddend are significantly modified
...
Hence, we do not use the  \DIFdelbegin \DIFdel{equilibrium, profile , }\DIFdelend \DIFaddbegin \DIFadd{profile (}\DIFaddend or {\tt GPEC}\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{) }\DIFaddend data within the three ELM-suppression/mitigation windows. Instead, we
interpolate across these windows using data taken on either side of the windows. \DIFaddbegin \DIFadd{The ultimate justification for this approach is the fact that, as can be seen from Figs.~\ref{fig5}, \ref{fig8}, and
\ref{fig11}, }{\tt \DIFadd{EPEC}} \DIFadd{simulations performed in the absence of the RMP (i.e., in the absence of RMP-generated electromagnetic torques) give very similar natural frequencies to simulations performed in the presence of the RMP, except
...
surfaces that lie very close to the LCFS\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend in other words,  surfaces that lie in the region $0.997 < {\mit\Psi}_N\leq 1.000$. \DIFdelbegin \DIFdel{Let }\DIFdelend \DIFaddbegin \DIFadd{(It should be
noted that }{\tt \DIFadd{EPEC}}\DIFadd{, like }{\tt \DIFadd{GPEC}}\DIFadd{, employs a straight field-line flux coordinate system, and is effectively
spectral in the poloidal and toroidal directions.)  }\DIFaddend $\overline{q_{95}}\equiv 5.2060-0.4457\,t$ \DIFdelbegin \DIFdel{represent }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend the
...
Combining this data  with modeling from a neutral particle simulation code\DIFaddbegin \DIFadd{, }\DIFaddend such as, for example, the  SOLPS-ITER code\DIFdelbegin \DIFdel{\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \cite{solps} ought to generate sufficient information to fully determine the
...
The upper panel of Fig.~\ref{fig5} shows the natural frequencies, \DIFaddbegin \DIFadd{reconstructed by }{\tt \DIFadd{EPEC}} \DIFadd{from profile data, }\DIFaddend in the absence of the RMP \DIFdelbegin \DIFdel{, of the }\DIFdelend \DIFaddbegin \DIFadd{(i.e., in the absence of
any RMP-induced electromagnetic torques in the plasma), of the }\DIFaddend various  $n=3$ tearing modes 
...
prior to the occurrence of the third ELM-suppression/mitigation window. The lower panel of Fig.~\ref{fig5} shows the natural frequencies, \DIFaddbegin \DIFadd{reconstructed by }{\tt \DIFadd{EPEC}} \DIFadd{from profile data, }\DIFaddend in the presence of the RMP \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(i.e., modified by the electromagnetic torques that develop in the plasma in response to the RMP), }\DIFaddend of the various $n=3$
...
The upper panel of Fig.~\ref{fig6} shows the \DIFaddbegin \DIFadd{predicted }\DIFaddend widths of the various vacuum magnetic island chains driven by the applied $n=3$ RMP in the pedestal region as functions of $
\overline{q_{95}}$.
...
of Fig.~\ref{fig6} shows the \DIFdelbegin \DIFdel{actual }\DIFdelend \DIFaddbegin \DIFadd{predicted }\DIFaddend widths of the various magnetic island chains driven by the applied $n=3$ RMP as functions of time\DIFaddbegin \DIFadd{, taking into
account the shielding of driven magnetic reconnection by plasma flow}\DIFaddend . It is apparent 
that the \DIFdelbegin \DIFdel{true }\DIFdelend \DIFaddbegin \DIFadd{shielded }\DIFaddend island widths are generally much smaller than the vacuum island widths\DIFdelbegin \DIFdel{, as a consequence of the shielding of driven magnetic
reconnection due to plasma flow}\DIFdelend . However, the shielding breaks down at various \DIFdelbegin \DIFdel{rational }\DIFdelend \DIFaddbegin \DIFadd{resonant }\DIFaddend surfaces when the associated natural frequency is zero (see the lower panel of
...
island chains driven by the RMP in the pedestal region as functions of $\overline{q_{95}}$ and time. (See Sect.~III.F in Ref.~\onlinecite{rf2}\DIFaddbegin \DIFadd{, as well as
Refs.~}\onlinecite{flat} \DIFadd{and }\onlinecite{flat1}\DIFaddend .) (Note that the energy diffusivity data shown in Fig.~\ref{fig3}  is used to calculate the
...
the critical island width for density flattening.\cite{flat} \DIFaddbegin \DIFadd{This distinction was not made in Ref.~}\onlinecite{rf2}\DIFadd{, which just used a
generic perpendicular diffusivity for momentum, energy, and particles.}\DIFaddend ) It can be seen that the driven magnetic island chains \DIFaddbegin \DIFadd{that do not lock to the RMP }\DIFaddend are generally too narrow
...
due to the applied RMP \DIFaddbegin \DIFadd{from such island chains}\DIFaddend . 
However, in those situations in which the shielding of driven
magnetic reconnection due to plasma flow breaks down \DIFaddbegin \DIFadd{at a given resonant surface }\DIFaddend (i.e., the true natural frequency becomes zero), \DIFaddbegin \DIFadd{a }\DIFaddend locked magnetic island \DIFdelbegin \DIFdel{chains are generated that are
 }\DIFdelend \DIFaddbegin \DIFadd{chain is generated that is
 }\DIFaddend wide enough to flatten both the density and the electron temperature profiles. 
...
Likewise, it is apparent from Fig.~\ref{fig9} that the \DIFdelbegin \DIFdel{true }\DIFdelend \DIFaddbegin \DIFadd{shielded }\DIFaddend island widths driven in the pedestal by the applied
...
the top of the pedestal, and also occur \DIFdelbegin \DIFdel{at about the correct times}\DIFdelend \DIFaddbegin \DIFadd{in approximately the correct time intervals}\DIFaddend . Note that overlapping island chains
...
It can be seen from Fig~\ref{fig13} that there is \DIFdelbegin \DIFdel{very little correlation between the major regions of density and temperature flattening }\DIFdelend \DIFaddbegin \DIFadd{no ELM suppression/mitigation window }\DIFaddend associated with the \DIFdelbegin \DIFdel{$n=3$ tearing modes driven by the  RMP and the  ELM-suppression}\DIFdelend \DIFaddbegin \DIFadd{$m=11$ mode.  
However, the  ELM suppression}\DIFaddend /mitigation windows \DIFdelbegin \DIFdel{. To be more exact, the regions  are not really located at the top of the pedestal, and occur at completely the wrong times}\DIFdelend \DIFaddbegin \DIFadd{associated with the $m=10$ and $m=9$ modes occur in approximately the correct
time intervals}\DIFaddend . 
...
Three cases are considered. \DIFdelbegin \DIFdel{In the }\DIFdelend \DIFaddbegin \DIFadd{These cases are summarized in Fig.~\ref{fig14}. (In this figure, the ELM/mitigation suppression windows are defined as the intersection of the regions
of density flattening and the top of the pedestal in Figs.~\ref{fig7}, \ref{fig10}, and \ref{fig13}.) In the }\DIFaddend first case, the natural frequencies of (stable) $n=3$ tearing modes, resonant in the plasma, are assumed to be
...
determined by the \DIFdelbegin \DIFdel{guiding-center flows }\DIFdelend \DIFaddbegin \DIFadd{${\bf E}\times {\bf B}$ velocities }\DIFaddend at the corresponding rational surfaces.\cite{heyn,paz1} 
...
The \DIFdelbegin \DIFdel{second case gives the best }\DIFdelend \DIFaddbegin \DIFadd{first case only leads to partial }\DIFaddend agreement between  the {\tt EPEC} simulations and the  observations\DIFdelbegin \DIFdel{. To be more
exact, the second case leads to the prediction thatcomparatively wide locked magnetic island chains, that locally flatten the density and temperature profiles, 
 are present at the top of the pedestal during the ELM-suppression}\DIFdelend \DIFaddbegin \DIFadd{; in that, the first ELM suppression}\DIFaddend /mitigation \DIFdelbegin \DIFdel{windows, and are absent otherwise. This prediction is
in accordance with the prevailing hypothesis that ELM suppression is due to the appearance of locked
...
large enough value to trigger peeling-ballooning modes.  \mbox{%DIFAUXCMD
\cite{d3d}
}\hspace{0pt}%DIFAUXCMD
}%DIFDELCMD < 

%DIFDELCMD <  %%%
\DIFdel{The third case gives by far the worst agreement between the }%DIFDELCMD < {\tt %%%
\DIFdel{EPEC}%DIFDELCMD < } %%%
\DIFdel{simulations and the  observations. }\DIFdelend \DIFaddbegin \DIFadd{window is
missing.  }\DIFaddend This lack of \DIFdelbegin \DIFdel{agreement is hardly surprising, given
that we have no reason to suppose that the response of a tokamak plasma to an RMP is correctly described by linear tearing mode theory.\mbox{%DIFAUXCMD
\cite{rf1} 
}\hspace{0pt}%DIFAUXCMD
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The first case only leads to partial agreement between  the }%DIFDELCMD < {\tt %%%
\DIFdel{EPEC}%DIFDELCMD < } %%%
\DIFdel{simulations and the  observations. This lack of }\DIFdelend complete agreement is somewhat
...
\DIFaddbegin \DIFadd{The second case gives the best agreement between the }{\tt \DIFadd{EPEC}} \DIFadd{simulations and the  observations. To be more
...
\cite{d3d}
 ...
 \DIFadd{The third case only leads to partial agreement between  the }{\tt \DIFadd{EPEC}} \DIFadd{simulations and the  observations; in that, the third ELM suppression/mitigation window is
missing. This lack of agreement is probably due to that fact that the response of a tokamak plasma to an RMP is not accurately described by linear tearing mode theory.\mbox{%DIFAUXCMD
\cite{rf1} 
}\hspace{0pt}%DIFAUXCMD
}
...
\DIFaddbegin \bibitem{lyons} \DIFadd{B.C.~Lyons, N.M.~Ferraro, C.~Paz-Solden, R.~Nazikian, and A.~Wingen, Plasma Phys.\ Contol.\ Fusion }{\bf \DIFadd{59}}\DIFadd{, 044001(2017).
}

\DIFaddend \bibitem{paz1} C.~Paz-Solden, R.~Nazikian, L.~Cui, B.C.~Lyons,  D.M.~Orlov, A.~Kirk, N.C.~Logan, T.H.~Osborne, W.~Suttrop, and D.B.~Weisberg, 
...
\DIFaddbegin \bibitem{ara} \DIFadd{G.~Ara,  B.~Basu, B.~Coppi, G.~Laval, M.N.~Rosenbluth, and B.V.~Waddell, Ann.\ Phys.\ (N.Y.) }{\bf \DIFadd{112}}\DIFadd{, 443 (1978). 
}

\DIFaddend \bibitem{nl1} R.~Fitzpatrick, and F.L.~Waelbroeck, Phys.\ Plasmas {\bf 12}, 022307 (2005).

\DIFaddbegin \bibitem{wat} \DIFadd{R.~Fitzpatrick, P.G.~Watson, and F.L.~Waelbroeck, Phys.\ Plasmas }{\bf \DIFadd{12}}\DIFadd{, 082510 (2005).
}

\DIFaddend \bibitem{nl2} R.~Fitzpatrick, Phys.\ Plasmas {\bf 25}, 042503 (2018).
...
\DIFaddbegin \bibitem{coppi} \DIFadd{B.~Coppi, J.M.~Greene, and J.L.~Johnson, Nucl.\ Fusion }{\bf \DIFadd{6}}\DIFadd{, 101 (1966).
}

\bibitem{ruth} \DIFadd{P.H.~Rutherford,  Phys.\ Fluids  }{\bf \DIFadd{16}}\DIFadd{, 1903 (1973).
}

\bibitem{pletz} \DIFadd{A.~Pletzer, and R.L.~Dewar, J.\ Plasma Physics }{\bf \DIFadd{45}}\DIFadd{, 427 (1991).
}

\DIFaddend \bibitem{am1} R.~Fitzpatrick, R.J.~Hastie, T.J.~Martin, and C.M.~Roach, Nucl.\ Fusion {\bf 33}, 1533 (1993).

\DIFaddbegin \bibitem{pletz1} \DIFadd{A.~Pletzer, A.~Bondeson, and R.L.~Dewar, J.\ Comp.\ Phys.\ {\bf 115}, 530 (1994).
}

\bibitem{tokuda} \DIFadd{S.~Tokuda, Nucl.\ Fusion }{\bf \DIFadd{41}}\DIFadd{, 1037 (2001).
}

\bibitem{brennan} \DIFadd{D.P.~Brennan, R.J.~La Haye, A.D.~Turnbull, M.S.~Chu, T.H.~Jensen, L.L.~Lao, T.C.~Luce, P.A.~Politzer, and E.J.~Strait,
Phys.\ Plasmas }{\bf \DIFadd{10}}\DIFadd{, 1643 (2003).
}

\bibitem{fura} \DIFadd{M.~Furukawa, and S.~Tokuda, Phys.\ Plasmas }{\bf \DIFadd{19}}\DIFadd{, 102511 (2012).
}

\DIFaddend \bibitem{am2} A.H.~Glasser, Z.R.~Wang, and J.-K.~Park, Phys.\ Plasmas {\bf 23}, 112506 (2016).
...
\DIFdelbegin  \bibitem{gim} \DIFdel{R.~Fitzpatrick, C.G.~Gimblett, and R.J.~Hastie, Plasma Phys.\ Control.\ Fusion {\bf 34}, 161 (1992).}
 \bibitem{haz} \DIFdel{R.D.~Hazeltine, and J.D.~Meiss, Rev.\ Mod.\ Phys.\ {\bf 121}, 1 (1985).}
\bibitem{rr} \DIFdel{P.H.~Rutherford,  Phys.\ Fluids, {\bf16}, 1903 (1973).}
\bibitem{wesson} \DIFdel{J.A.~Wesson, private communication (1993).}
  \bibitem{ara}  \DIFdel{G.~Ara,  B.~Basu, B.~Coppi, G.~Laval, M.N.~Rosenbluth, and B.~Waddell, Ann.\ Phys.\ (N.Y.) {\bf 112}, 443 (1978). }
  \bibitem{wat} \DIFdel{R.~Fitzpatrick, P.G.~Watson, and F.L.~Waelbroeck, Phys.\ Plasmas {\bf 12}, 082510 (2005).}
\DIFdelend \bibitem{rffig} \DIFdel{R.~Fitzpatrick, Phys.\ Plasmas {\bf 10}, 2304 (2003).}

  \DIFaddbegin \bibitem{flat1} \DIFadd{N.N.~Gorelenkov, R.V.~Budny, Z.~Chang, M.V.~Gorelenkova, and L.E.~Zakharov, Phys.\ Plasmas {\bf 3}, 3379 (1996).}\DIFaddend .

\end{thebibliography}

\DIFdelbegin %DIFDELCMD < \appendix
%DIFDELCMD < %%%
\section{\DIFdel{Linear Versus Nonlinear Tearing Mode Theory in Tokamak Plasmas}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{appa}
%DIFDELCMD < %%%
\subsection{\DIFdel{Introduction}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdel{The discussion in this Appendix outlines certain facts pertaining
...
shorter transport timescale.\mbox{%DIFAUXCMD
\cite{transp,gim}
}\hspace{0pt}%DIFAUXCMD
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{A linear tearing mode is a helical instability of a tokamak plasma equilibrium that is related to a shear-Alfv\'{e}n wave.\mbox{%DIFAUXCMD
\cite{haz} }\hspace{0pt}%DIFAUXCMD
...
the hybrid timescale $\tau_A\ll \tau_A^{2/5}\,\tau_R^{3/5}\ll \tau_R$.\mbox{%DIFAUXCMD
\cite{fkr} }\hspace{0pt}%DIFAUXCMD
Note that  the
...
equilibrium localized around the rational surface.\mbox{%DIFAUXCMD
\cite{ruth}
}\hspace{0pt}%DIFAUXCMD
...
determines the phase velocity of a nonlinear magnetic island chain (see Ref.~}%DIFDELCMD < \onlinecite{nl2}%%%
...
an irrelevant timescale in nonlinear tearing mode theory, and
is also  very much shorter than the timescales on which
physical quantities of interest actually evolve. It should be noted that linear tearing layers in present-day tokamak plasmas are sufficiently thin that by the time a tearing mode is detectable it is already in the nonlinear regime (i.e., the island width exceeds the linear layer width).\mbox{%DIFAUXCMD
\cite{wesson} }\hspace{0pt}%DIFAUXCMD
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Natural Frequency}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{natural}
%DIFDELCMD < %%%
\DIFdel{The natural frequency of a (stable) tearing mode is
the helical phase velocity with which it would propagate
were it actually unstable.\mbox{%DIFAUXCMD
\cite{rfa} }\hspace{0pt}%DIFAUXCMD
...
\caption{Top Panel: The red, green, blue,  \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL cyan\DIFaddbeginFL \DIFaddFL{, and magenta }\DIFaddendFL curves show the electron number density ($10^{19}\,{\rm m}^{-3}$),
electron temperature (keV), (thermal) ion temperature (keV), 
 \DIFdelbeginFL \DIFdelFL{and
 }\DIFdelendFL C-VI ion number density  ($10^{18}\,{\rm m}^{-3}$)\DIFaddbeginFL \DIFaddFL{, and flux-surface averaged neutral density ($10^{17}\,{\rm m}^{-3}$)   }\DIFaddendFL profiles, respectively,  in  DIII-D discharge \#145380 at time $t=2500$ ms. Middle Panel:  
 ...
 the perpendicular momentum \DIFaddbeginFL \DIFaddFL{($\chi_\phi$)}\DIFaddendFL , energy \DIFaddbeginFL \DIFaddFL{($\chi_E$)}\DIFaddendFL , and particle \DIFaddbeginFL \DIFaddFL{($D_\perp$) }\DIFaddendFL diffusivity profiles, respectively, 
...
effective plasma boundary for the \DIFaddbeginFL {\tt \DIFaddendFL GPEC\DIFaddbeginFL } \DIFaddendFL and \DIFaddbeginFL {\tt \DIFaddendFL EPEC\DIFaddbeginFL } \DIFaddendFL calculations
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=6in]{fig5.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=7in]{fig5.pdf}
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=6in]{fig6.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=7in]{fig6.pdf}
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=6in]{fig7.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=7in]{fig7.pdf}
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=6in]{fig8.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=7in]{fig8.pdf}
\DIFaddendFL \caption{Top Panel: $n=3$ natural frequencies, in absence of RMP, as functions of the least-squares linear fit to $q_{95}$ versus time
...
Bottom Panel:  $n=3$ natural frequencies, in presence of RMP, as functions of time
in   DIII-D discharge \#145380, assuming that the natural frequency is  determined by the \DIFaddbeginFL \DIFaddFL{local }\DIFaddendFL ${\bf E}\times {\bf B}$
\DIFdelbeginFL \DIFdelFL{frequency}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{velocity}\DIFaddendFL . The red, green, blue, yellow, cyan, magenta, brown, pink,
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=6in]{fig9.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=7in]{fig9.pdf}
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=6in]{fig10.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=7in]{fig10.pdf}
\DIFaddendFL \caption{Top Panel: Density flattening widths associated with induced $n=3$ magnetic island  chains as functions of the least-squares linear fit to $q_{95}$ versus time
in   DIII-D discharge \#145380, assuming that the natural frequency is  determined by the \DIFaddbeginFL \DIFaddFL{local }\DIFaddendFL ${\bf E}\times {\bf B}$
\DIFdelbeginFL \DIFdelFL{frequency}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{velocity}\DIFaddendFL .
Bottom Panel:  Electron temperature flattening widths associated with induced $n=3$ magnetic island chains as functions of time
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=6in]{fig11.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=7in]{fig11.pdf}
\DIFaddendFL \caption{Top Panel: $n=3$ natural frequencies, in absence of RMP, as functions of the least-squares linear fit to $q_{95}$ versus time
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=6in]{fig12.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=7in]{fig12.pdf}
\DIFaddendFL \caption{Top Panel: $n=3$ vacuum island widths as functions of the least-squares linear fit to $q_{95}$ versus time
in   DIII-D discharge \#145380.
Bottom Panel:  $n=3$ island widths as functions of time
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=6in]{fig13.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=7in]{fig13.pdf}
\DIFaddendFL \caption{Top Panel: Density flattening widths associated with induced $n=3$ magnetic island  chains as functions of the least-squares linear fit to $q_{95}$ versus time
in   DIII-D discharge \#145380, assuming that the natural frequency is determined by linear layer physics.
Bottom Panel:  Electron temperature flattening widths associated with induced $n=3$ magnetic island chains as functions of time
in   DIII-D discharge \#145380, assuming that the natural frequency is determined by linear layer physics. The \DIFdelbeginFL \DIFdelFL{blue, }\DIFdelendFL yellow, cyan, magenta, brown, pink,
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=4in]{fig14.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=3in]{fig14.pdf}
\DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Schematic diagram showing the perturbed plasma current and vorticity patterns around the magnetic X- and O-points }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Summary }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{a linear and a nonlinear tearing mode}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{results}\DIFaddendFL . The \DIFdelbeginFL \DIFdelFL{horizontal axis measures radial distance from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{yellow vertical bands indicate }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{rational
surface}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{experimental $n=3$ ELM-suppression/mitigation windows in DIII-D discharge \#145380}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{whereas }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{as functions  of }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{vertical axis measures distance along equilibrium magnetic field-lines}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{least-squares linear fit to $q_{95}$ versus time}\DIFaddendFL . 
\DIFdelbeginFL \DIFdelFL{Red }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The brown, magenta, }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{blue
correspond to positive and negative current}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{cyan bars show the  ELM suppression}\DIFaddendFL /\DIFdelbeginFL \DIFdelFL{vorticity values}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{mitigation windows predicted by the }{\tt \DIFaddFL{EPEC}} \DIFaddFL{code that are associated with locked $m=11/n=3$}\DIFaddendFL , \DIFaddbeginFL \DIFaddFL{$10/3$, and $9/3$, magnetic island chains, }\DIFaddendFL respectively\DIFaddbeginFL \DIFaddFL{, 
driven at the top of the pedestal}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{(See Figs.~7 and 10 in 
Ref.~}%DIFDELCMD < \onlinecite{rffig}%%%
\DIFdelFL{.}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The three cases considered are such that the natural frequency at a given resonant surface is determined by:
1}\DIFaddendFL ) \DIFaddbeginFL \DIFaddFL{the local ion flow; 2) the local ${\bf E}\times {\bf B}$ velocity; 3) the local electron flow.}\DIFaddendFL }\label{fig14}
\end{figure}
