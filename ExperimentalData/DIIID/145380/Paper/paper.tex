...
the local electron fluid at the resonant surface (see the discussion in Sect.~\ref{natural}).\cite{hender,cole} Hence, if the response of an H-mode tokamak plasma to an applied RMP
...
the local ion fluid at the rational surface (again, see the discussion in Sect.~\ref{natural}).\cite{nl1,nl2,nl3}
...
the kink response of the plasma is calculated exactly using the {\tt GPEC} code.\cite{ipec,gpec}
...
plasma current $I_p$ is slowly ramped over a 2 s interval to scan the magnetic safety-factor.\cite{d3d,d3d2} The majority ions are Deuterium, whereas the
minority ions are Carbon~VI.
Figure~\ref{fig1} gives an overview of this discharge. Note that the pedestal density is relatively high ($n_{e\,{\rm ped}}\sim 3\times 10^{19}\,{\rm m}^{-3}$). 
...
normalized poloidal magnetic flux defined such that the magnetic axis corresponds to ${\mit\Psi}_N=0$, 
...
plasma profile data is available every $50$ ms. The equilibrium and profile data at a given point in time is interpolated from
the measured data. In fact, quantities that depend on the plasma equilibrium and profiles are recalculated every 1 ms.  Data from the
{\tt GPEC} code is available every 100 ms. As before, the {\tt GPEC} data at a given point in time is interpolated from the
available {\tt GPEC} data, and any quantity that depends on this data is recalculated every 1 ms. There is one major exception to
...
within the three ELM-suppression windows (presumably because wide magnetic island chains are driven in the pedestal). 
...
constant at about 1.0. For the sake of practicality,  the {\tt GPEC} and {\tt EPEC} calculations described in this paper ignore rational
surfaces that lie very close to the LCFS: in other words,  surfaces that lie in the region $0.997 < {\mit\Psi}_N\leq 1.000$. 

Note that every quantity used in our theoretical model is derived from experimental measurements taken in DIII-D discharge \#145380, with one
exception. The exception is the neutral particle data. Unfortunately, no neutral particle data is available for DIII-D discharge \#145380.
Hence, we employ neutral particle data derived from a much earlier discharge (see Ref.~\onlinecite{rf2}).\cite{neutral}
In future, data from the new {\tt LLAMA} (Lyman Alpha Measurement Apparatus) system will be available in DIII-D discharges.\cite{llama}
...
neutral particle parameters required by the {\tt EPEC} code.  

\subsection{Case 1: $\varpi=\varpi_{\rm nonlinear}$}
This first case that we shall consider is that in which the natural frequency is determined by nonlinear
tearing mode theory: in other words, the natural frequency is specified by Eq.~(\ref{e2}). 

The upper panel of Fig.~\ref{fig5} shows the natural frequencies of the various  $n=3$ tearing modes 
 that are resonant in the pedestal. It can be seen that the natural frequency of the $m=11$ harmonic has its main zero crossing in the middle
of the first ELM-suppression window. Likewise, the natural frequency of the $m=10$ harmonic has its main zero crossing in the middle of the
second ELM-suppression window. However, the natural frequency of the $m=9$ harmonic has its main zero crossing slightly
before the third ELM-suppression window. The lower panel of Fig.~\ref{fig5} shows the actual phase velocities of the various $n=3$
tearing modes that are resonant in the pedestal. It can be seen that if the magnitude of the natural frequency falls below about
$5\,{\rm krad/s}$ then the associated tearing mode locks to the RMP (i.e., its phase velocity becomes zero). 

\subsection{Case 2: $\varpi=\varpi_{EB}$}

\subsection{Case 3: $\varpi=\varpi_{\rm linear}$}
...
(${\mit\Psi}_N=0.997$), and the true plasma boundary (${\mit\Psi}_N=1.00$), respectively. The yellow vertical bands indicate the ELM-suppression windows. The horizontal dotted lines indicate the safety-factors at the various $n=3$ rational surfaces. 
} \label{fig4}
\end{figure}
