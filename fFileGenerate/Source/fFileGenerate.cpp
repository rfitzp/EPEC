...
extern "C" void NameListRead (int* FLUX_NTOR, int* FLUX_MMIN, int* FLUX_MMAX, double* FLUX_PSIPED, double* FLUX_PSIRAT, double* FLUX_PSILIM,
			      int* NEO_INTF, int* NEO_INTC, int* NEO_IMPURITY, int* NEO_NEUTRAL, int* NEO_EXB, int* NEO_CATS, int* NEO_NTYPE,
			      double* NEO_NN, double* NEO_LN, double* NEO_YN, double* TSTART, double* TEND, double* DT); 
...
void fFileGenerate ()
{
  ...
  int	 NEO_EXB;        // Set ExB frequency type in NEOCLASSICAL 
  ...
  if (TEND < TSTART)
    {
      printf ("fFILEGENEReATE:: Error - TEND must be greater than TSTART\n");
      exit (1);
    }
  if (DT < 0.)
    {
      printf ("fFILEGENEReATE:: Error - DT must be positive\n");
      exit (1);
    }
  ...
  FILE* namelist = fopen ("Outputs/InputParameters.txt", "w");
  ...
  // Print exit message
  printf ("**********************************************************************\n");
  printf ("PROGRAM fFILEGENERATE:: Normal termination: Wall time = %11.4e s\n", time_spent);
  printf ("**********************************************************************\n");
}
