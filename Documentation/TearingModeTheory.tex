...
Consider a large aspect-ratio tokamak plasma equilibrium with whose magnetic flux-surfaces map out (almost) concentric circles in the poloidal plane. 
...
The tearing perturbation is governed by the linearized version of Equation~(\ref{e84}), which yields
...
and the boundary conditions (\ref{e101})--(\ref{e103}). Note that, in general, $\psi$ is continuous across the resonant surface,
...
thin current sheet  at the resonant surface. This current sheet can only be resolved by retaining more terms in the perturbed plasma equation of motion, (\ref{e96}). 
...
In general, $\psi$ is continuous across the wall, whereas $\partial\psi/\partial r$ is discontinuous. The discontinuity in $\partial\psi/\partial r$ is caused by the currents induced in the wall. The complex quantity ${\mit\Psi}_w(t)$ determines the amplitude and
...
\end{equation}
parameterizes the amplitude and phase of the current sheet flowing in the wall.
...
\end{equation}
Suppose that the radial thickness of the wall is $\delta_w\ll r_w$. The previous two equations yield
...
\end{equation}
Here, $\tau_s$ is the {\em reconnection time}, whereas $\varpi_s$ is the so-called {\em natural frequency}. 
...
helical current possesing $m$ periods in the poloidal direction, and $n$ periods in the `toroidal' direction. Let the current density in the coil
take the form
...
In general, $\psi$ is continuous across the field-coil, whereas $\partial\psi/\partial r$ is discontinuous. The discontinuity in $\partial\psi/\partial r$ is caused by the currents flowing in the coil. The complex quantity ${\mit\Psi}_c(t)$ determines the amplitude and
...
can be written
\begin{align}
T_\theta(r) &= \oint\oint R_0\,r^{\,2}\,{\bf j}\times {\bf B}\cdot {\bf e}_\theta\,d\theta\,d\phi,\\[0.5ex]
T_\phi(r) &= \oint\oint R_0^{\,2}\,r\,{\bf j}\times {\bf B}\cdot {\bf e}_z\,d\theta\,d\phi,
\end{align}
respectively. However, according to Equation~(\ref{e80}),
