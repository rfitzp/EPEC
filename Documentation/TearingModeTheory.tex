...
{\mit\Delta\Psi}_s = \tau_s\left(\frac{d{\mit\Psi}_s}{dt} + {\rm i}\,\omega\,{\mit\Psi}_s\right).
\end{equation}
Here, $\tau_s$ is the {\em reconnection time}\/ (i.e., the typical timescale on which magnetic reconnection takes place in the
resonant layer surrounding the resonant surface), whereas $\omega$ is the phase velocity 
of the tearing mode when the wall is perfectly conducting. The
phase velocity  is non-zero because the reconnected flux in the resonant layer is convected by the local plasma
...
surface will modify the convection velocity. Consequently, the phase velocity of the tearing mode can be written
\begin{equation}\label{e200}
 \omega(t) = \omega_{0} + ({\bf k}\cdot {\mit\Delta}{\bf V}_i)_{r=r_s}=
 \omega_{0} + m\,{\mit\Delta\Omega}_\theta(r_s,t) - n\,{\mit\Delta\Omega}_\varphi(r_s,t),
\end{equation}
where $\omega_{0}$ is the unperturbed phase velocity. The previous expression is known as the {\em no-slip constraint}\/ (Fitzpatrick 1993), and has been verified experimentally (Vahala, et al.\ 1980). 
It follows from Equations~(\ref{e190})
and (\ref{e191}) that
\begin{equation}\label{e201}
\omega(t) = \omega_{0} - \sum_{p=1,\infty}\left[\alpha_p(t)+\beta_p(t)\right].
\end{equation}
(See Section~\ref{stb}.)
...
{\mit\Delta\hat{\Psi}}_s\equiv \tau_s\left(\frac{d\hat{\mit\Psi}_s}{dt} + {\rm i}\,\omega\,\hat{\mit\Psi}_s\right) &= E_{ss}\,\hat{\mit\Psi}_s + E_{sw}\,\hat{\mit\Psi}_w,\\[0.5ex]
\tau_w\,\frac{d\hat{\mit\Psi}_w}{dt} &= E_{sw}\,\hat{\mit\Psi}_s +\tilde{E}_{ww}\,\hat{\mit\Psi}_w
+\hat{I}_c,\label{e203}\\[0.5ex]
\omega(t)& = \omega_{0} - \sum_{p=1,\infty}\left[\alpha_p(t)+\beta_p(t)\right],\label{e210a}\\[0.5ex]
(1+2\,q_s^{\,2})\,\frac{d\alpha_p}{dt} + \left(\frac{1}{\tau_{\theta\,s}}+\frac{j_{1\,p}^{\,2}}{\tau_{M}}\right)\alpha_p
&= \frac{m^{2}\,[y_p(r_s/a)]^{\,2}}{\tau_A^{\,2}\,\epsilon_a^{\,2}\,[J_2(j_{1\,p})]^{\,2}}
...
phase velocity, $\omega_{0}$---that can only be determined by solving for the plasma response in the radially-thin resonant layer surrounding the resonant surface. Unfortunately, determining the resonant plasma response is not a straightforward task. The
...
\end{align}
where $\hat{t}=V_A\,t/r_s$, $\hat{\omega}=r_s\,\omega/V_A$, and $\omega$ is the phase velocity of the tearing mode in the laboratory frame. 
...
surface ($\hat{x}=0$). The chain has $m$ periods in the poloidal direction, $n$ periods in the toroidal direction, and propagates at the phase velocity $\omega$. The maximum full (as opposed to half) radial width of the magnetic separatrix is 
...
{\mit\Delta} = {\rm i}\,\tau_s\,(\omega_{0}+ {\mit\Delta}\omega_E),
\end{equation}
...
\hat{\mit\Psi}_s=\frac{\tilde{I}_c}{(-{\mit\Delta}_{nw})+ {\rm i}\,\tau_s\,(\omega_{0}+ {\mit\Delta}\omega_E)}.
\end{equation}
Now, in a conventional ohmically-heated tokamak plasma, $\tau_s\,\omega_{0}\gg (-{\mit\Delta}_{nw})$. Consequently, the
...
where 
\begin{align}
v&=\frac{\omega_{0}+{\mit\Delta}\omega_E}{\omega_{0}},\\[0.5ex]
T_{\rm VS}(v) &= 1- v,\\[0.5ex]
T_{\rm EM}(v) &= \frac{1}{4}\,\frac{v}{\zeta_s^{\,2}+v^2}
\left(\frac{|\tilde{I}_c|}{\tilde{I}_{c\,{\rm crit}}}\right)^2,\\[0.5ex]
\zeta_s &= \frac{(-{\mit\Delta}_{nw})}{\tau_s\,\omega_{0}}\ll 1,\\[0.5ex]
\tilde{I}_{c\,{\rm crit}}&= (\epsilon_s\,s_s)\,(\omega_{0}\,\tau_H)\left/
...
penetration increases linearly with the unperturbed plasma rotation at the resonant surface, which is parameterized by $\omega_{0}$. 
...
shall assume that $T_i=T_e=T_0$, $\omega_{0}=\omega_\ast$, and $-(d\ln p/dr)_{r_s} = 1/r_s$. We shall also neglect ${\cal O}(1)$
...
It is easily demonstrated that
\begin{align}
\omega_{0}\,\tau_H&\simeq \left(\frac{q_s}{\epsilon_s}\right)^2\rho_\ast\,\beta_\ast^{\,1/2},\\[0.5ex]
...
\end{align}
Here, $\omega = \hat{\omega}\,V_A/r_s$ is the phase velocity of the island chain in the
laboratory frame,
\begin{equation}
s = {\rm sgn}(\hat{x}),
\end{equation}
...
\end{align}
Here, use has been made of Equations~(\ref{e317a}), (\ref{e318a}), (\ref{e481}), and (\ref{e482}). Note that $d/dt$ in Equation~(\ref{e559}) represents a time derivative in the island rest frame. 
...
\end{equation}
where $d/dt$ represents a time derivative in the island rest frame. 
...
\subsection{Island Phase Velocity}\label{sphase1}
...
\subsection{Discussion}
The phase velocity of an isolated tearing mode is determined by the
...
across the separatrix. The island phase velocity is determined by
...
its much greater perpendicular viscosity. Hence, a nonlinear magnetic island chain is convected by the ion fluid in the vicinity of the resonant surface, because this choice of phase velocity minimizes the ion fluid velocity gradient across the separatrix (Fitzpatrick 2000). Incidentally, there is clear experimental evidence that wide magnetic island chains in tokamak plasmas rotate in the ion
...
Assuming that the tearing mode possesses the phase velocity $\omega$, and that $\hat{I}_c=0$, 
...
stabilization when the phase velocity of the mode is sufficiently small that the associated perturbed magnetic field  can penetrate through the wall.  The third and fourth terms represent the destabilizing effect of the ion polarization
...
Equations~(\ref{e210a}) and (\ref{e574}) imply that
\begin{equation}
\omega = \omega_{0} -\sum_{p=1,\infty}(\alpha_p+\beta_p),
\end{equation}
where $\omega_0=\omega_{\perp\,i}$ is the island phase velocity in the absence of the resistive wall. In the following, $\omega_0$ is
assumed to be positive for the sake of definiteness. The modifications to the analysis when $\omega_0$ is
negative are trivial. 
The previous equation can be combined with Equations~(\ref{e205}), (\ref{e206}), (\ref{e417a}), (\ref{e418a}), (\ref{e456}),
and (\ref{e584}) to give the following torque balance equation:
\begin{equation}\label{e588}
\omega_0 - \omega = \frac{1}{4}\,[{\mit\Delta}_{nw}(0)-{\mit\Delta}_{pw}(0)]
...
where
\begin{equation}
\tau_V = 2\,\ln\!\left(\frac{a}{r_s}\right)\tau_\varphi + \left(\frac{q_s}{\epsilon_s}\right)\left(\frac{\tau_i\,\tau_\varphi}{\mu_{00}^i}\right)^{1/2}.
\end{equation}
The left-hand side of Equation~(\ref{e588}) represents the viscous restoring torque that acts to prevent changes in the
island chain's phase velocity. The right-hand side represents the electromagnetic braking torque acting on the island chain due to eddy currents induced in the
resistive wall. 

Let
\begin{align}
x &= \frac{x}{x_{pw}},\\[0.5ex]
y &= \frac{\omega}{\omega_0},\\[0.5ex]
...
where
\begin{align}
\alpha_l & = \omega_0\,\tau_{LR},\\[0.5ex]
\beta_l &= 1 - \frac{{\mit\Delta}_{pw}(0)}{{\mit\Delta}_{nw}(0)},\\[0.5ex]
\gamma_l&= \frac{I_2}{{\mit\Delta}_{nw}(0)}\left(\frac{q_s}{\epsilon_s}\right)^2\left(\frac{c_\beta}{1+\tau}\right)
\left(\frac{\rho_\beta}{r_s}\right)\left(\frac{\tau_\varphi\,\tau_H\,\omega_0}{\tau_V}\right)\left(\frac{r_s}{w_{pw}}\right)^2,\\[0.5ex]
\delta_l&= \frac{I_3}{{\mit\Delta}_{nw}(0)}\left(\frac{q_s}{\epsilon_s}\right)^4\left(\frac{\tau_\varphi\,\tau_H\,\omega_0}{\tau_V}\right)^2\left(\frac{r_s}{w_{pw}}\right),\\[0.5ex]
\epsilon_l &= \frac{\beta_l}{4\,{\mit\Delta}_{pw}(0)}\left(\frac{\epsilon_s}{q_s}\right)^2\left(\frac{\tau_{LR}\,\tau_V}{\tau_H^{\,2}}\right)\left(\frac{w_{pw}}{r_s}\right)^4.
\end{align}
...
critical value, $x_h$---at which point the island phase velocity has been reduced to {\em half}\/ of its original value---and
...
Note that, in the presence of a resistive wall, there is a forbidden band of island phase velocities: $\omega_0/\alpha_l < \omega< \omega_0/2$. The existence of this forbidden band has been verified experimentally (Gates \& Hender 1996).
...
Rotation braking due to interaction with a resistive wall can reduce the phase velocity of a tearing mode
by a large factor, but it cannot reduce the velocity to zero (because the braking torque is zero when $\omega=0$).
...
\left(\frac{{\mit\Delta}_{nw} - {\Delta}_{pw}}{E_{sw}}\right)\frac{\hat{I}_c}{\hat{\mit\Psi}_s}.
\end{equation}
...
\begin{equation}
w_u = \left[\frac{4}{{\mit\Delta}_{pw}(0)}\left(\frac{q_s}{\epsilon_s}\right)^2\left(\frac{\tau_H^{\,2}\,\omega_0}{\tau_V}\right)\right]^{1/4}r_s.
\end{equation}
The left-hand side of Equation~(\ref{e613}) represents the viscous restoring torque that acts to prevent changes in the island
chain's phase velocity. The right-hand side represents the electromagnetic locking torque exerted on the island chain by
