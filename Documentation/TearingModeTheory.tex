...
\section{Reduced Drift-MHD Resonant  Response Model}\label{ff}
\subsection{Introduction}
...
physics of  compressible-Alfv\'{e}n waves from them altogether, in the process converting a drift-magnetohydrodynamical (MHD) response model
into a so-called {\em reduced}\/ drift-MHD response model (Strauss 1976; Hazeltine, Kotschenreuther \& Morrison 1985; 
Furuya, Yagi \& Itoh 2003). The aim of
...
\subsection{Introduction}
The aim of this section is to employ the reduced, drift-MHD, four-field, resonant, plasma response model derived in Section~\ref{ff} to determine the {\em linear}\/ response of the resonant layer to external perturbations.
...
N(\hat{x},\zeta,t) &= -\hat{V}_\ast\,\hat{x}+\left(\frac{1+\tau}{\tau}\right)\!\tilde{N}(\hat{x})\,{\rm e}^{\,{\rm i}\,(\zeta-\hat{\omega}\,\hat{t})},\\[0.5ex]
V(\hat{x},\zeta,t) &= \hat{V}_\parallel + \left(\frac{1+\tau}{\tau}\right)\!\tilde{V}(\hat{x})\,{\rm e}^{\,{\rm i}\,(\zeta-\hat{\omega}\,\hat{t})},\\[0.5ex]
...
valid when the width of the magnetic island chain at the resonant surface is much less than the linear layer width. 
In the opposite limit, in which the island width greatly exceeds the layer width, a nonlinear approach is required. 
The aim of this section  is to employ the reduced, drift-MHD, four-field, resonant, plasma response model derived in Section~\ref{ff} to 
construct a nonlinear resonant response model. 

It is convenient to set the normalization scale-length, $l$, in our four-field response model equal to the minor radius of the resonant surface, $r_s$. It is also convenient to work in a frame of reference that corotates with the island chain. In this reference frame, the
normalized reconnected flux, $\hat{\mit\Psi}_s(\hat{t})$, is assumed to be a positive real quantity. 
Furthermore, the four fields,
$\hat{\psi}$, $\phi$, $N$, and $V$, as well as the auxiliary field, $J$, are assumed to only be functions of the
normalized time, $\hat{t}= t\,V_A/r_s$, the normalized radial coordinate $\hat{x} = (r-r_s)/r_s$, and the
helical angle $\zeta= m\,\theta-n\,\varphi$. 
...
in the limit $|\hat{x}|/\hat{w}\gg 1$ (i.e., many island widths from the resonant surface), we expect 
\begin{align}
...
\end{equation}
The parameter $\hat{V}_E'$ is included in the analysis to take into account the fact that the plasma rotation profile in the outer region develops a
...
T &= \omega_\ast\,t.
\end{align}
Thus, $|X|\sim {\cal O}(1)$ in the immediate vicinity of the island chain. It is helpful to define the
...
\end{align}
The four-field model introduced in Section~\ref{fourf} rescales to give
...
\frac{\partial(\partial_X^{\,2}\,{\mit\Phi})}{\partial T} &= \partial_X\!\!\left\{{\mit\Phi} + \frac{{\cal N}}{1+\tau},{\mit\Psi}\right\}
+ [{\cal J},{\mit\Psi}]+ \epsilon_\chi \,\partial_X^{\,4}\!\left({\mit\Phi} + \frac{{\cal N}}{1+\tau}\right),\\[0.5ex]
\frac{\partial {\cal N}}{\partial T} &= \{{\mit\Phi}, {\cal N}\} + \{{\cal V},{\mit\Psi}\} + \epsilon_d\,\epsilon_n\,\{{\cal J},{\mit\Psi}\}\nonumber\\[0.5ex]&\phantom{=}
+ \epsilon_D \!\left[\left(\frac{w}{w_c}\right)^4\,\{\{{\cal N},{\mit\Psi}\}, {\mit\Psi}\}+\partial_X^{\,2}{\cal N}\right],\\[0.5ex]
\epsilon_d\,\frac{\partial {\cal V}}{\partial T} &= \epsilon_d\,\{{\mit\Phi},{\cal V}\}+\{{\cal N}, {\mit\Psi}\} + \epsilon_d\,\epsilon_\chi\,\partial_X^{\,2} {\cal V},\\[0.5ex]
...
and the critical island width, $w_c$, is defined in Equation~(\ref{wcdef}). 
...
{\mit\Phi} &\rightarrow v\,X +\frac{ s\,v'\,X^{\,2}}{2},\\[0.5ex]
{\cal N}&\rightarrow X,\\[0.5ex]
...
\end{align}
In the following, it is assumed that $v\sim{\cal O}(1)$ and $|v'|\ll 1$. Note that the boundary conditions (\ref{e483})--(\ref{e487}),
