...
v'(T) &= -\frac{\hat{w}}{2\,\omega_\ast}\left[r\,\frac{d\omega_E}{dr}\right]_{r_{s-}}^{r_{s+}}.
\end{align}
...
\begin{align}
V_\theta(x,\zeta,T) &= -\frac{\omega_\ast}{m}\,x\,M({\mit\Omega},T),\\[0.5ex]
V_{\theta\,i}(x,\zeta,T)& = -\frac{\omega_\ast}{m}\,x\,Y_i({\mit\Omega},T)\\[0.5ex]
V_{\theta\,e}(x,\zeta,T)& = -\frac{\omega_\ast}{m}\,x\,Y_e({\mit\Omega},T),
\end{align}
where
\begin{align}
...
Here, we have made use of the fact that $Y_i=0$ within the magnetic separatrix of the island chain. We have also demanded that the ion
...
MHD and electron fluid velocities are  discontinuous across the separatrix. (See Section~\ref{s8.6}.)

In the limit ${\mit\Omega}\gg 1$, the previous equation yields
\begin{equation}
Y_i\simeq v' -\left. \frac{v'}{\sqrt{2\,{\mit\Omega}}}\right/\int_1^\infty \frac{d{\mit\Omega}}{\langle X^4\rangle}.
\end{equation}
Hence, it follows from Equations~(\ref{e518}) and (\ref{e526}) that
\begin{equation}
M\simeq v' -\left. \frac{v'}{\sqrt{2\,{\mit\Omega}}}\right/\int_1^\infty \frac{d{\mit\Omega}}{\langle X^4\rangle} -\left(\frac{1}{1+\tau}\right)
\frac{1}{\sqrt{2\,{\mit\Omega}}}
\end{equation}
in the same limit. A comparison with Equation~(\ref{e506}) reveals that
\begin{equation}
v = -\frac{1}{1+\tau} - v'\left/\int_1^\infty \frac{d{\mit\Omega}}{\langle X^4\rangle}\right.,
\end{equation}
or 
\begin{equation}\label{e534}
\omega = \omega_{\perp\,i} -v'\,\omega_\ast\left/\int_1^\infty \frac{d{\mit\Omega}}{\langle X^4\rangle}\right.,
\end{equation}
where
\begin{equation}
\omega_{\perp\,i} = \omega_E + \omega_{\ast\,i},
\end{equation}
and use has been made of Equations~(\ref{omegai}), (\ref{omega}), and (\ref{e488}). 
...
\end{equation}
Finally, the previous equation can be combined with Equation~(\ref{e534}) to give
\begin{equation}
\omega=\omega_{\perp\,i} + I_1\left(\frac{\tau_\varphi}{\tau_H^{\,2}}\right)\left(\frac{W}{r_s}\right)^5{\rm Im}\!\left(\frac{{\mit\Delta}\hat{\mit\Psi}_s}{\hat{\mit\Psi}_s}\right),
\end{equation}
where
\begin{equation}
I_1 = 2^{-12}\left/\int_1^{\infty} \frac{d{\mit\Omega}}{\langle X^4\rangle}\right. =3.584\times 10^{-4}. 
\end{equation}
Here, use has been made of Equations~(\ref{e479}), (\ref{e480}), and (\ref{e482}). 
...
\nonumber\\[0.5ex]&\phantom{=}+\frac{4}{\epsilon_\eta\,\hat{w}}\,\frac{d(\ln \hat{w}^2)}{dT}\int_{-1}^\infty \frac{\langle \cos\zeta\rangle^2}{\langle 1\rangle}\,d{\mit\Omega},\label{e547}
\end{align}
where
\begin{equation}
...
\end{equation}
Here, we have neglected terms that are quadratic in the small parameter $v'$. Finally, Equations~(\ref{e545}) and (\ref{e547})
can be combined to give (Waelbroeck \& Fitzpatrick 1997)
\begin{equation}
{\rm Re}\!\left(\frac{{\mit\Delta}\hat{\mit\Psi}_s}{\hat{\mit\Psi}_s}\right)= I_2\,\tau_R\,\frac{d}{dt}\!\left(\frac{W}{r_s}\right)
- I_3\left(\frac{c_\beta}{1+\tau}\right)\left(\frac{d_\beta}{r_s}\right)\left(\frac{L_s}{L_n}\right)\left(\frac{\tau_\varphi}{\tau_H}\right)\left(\frac{W}{r_s}\right)^2 {\rm Im}\!\left(\frac{{\mit\Delta}\hat{\mit\Psi}_s}{\hat{\mit\Psi}_s}\right),
\end{equation}
where
\begin{align}
I_2 &= 2\int_{-1}^\infty \frac{\langle \cos\zeta\rangle^2}{\langle 1\rangle}\,d{\mit\Omega} =0.8227,\\[0.5ex]
I_3&= 2^{-6}\int_1^\infty d_{\mit\Omega}\!\left(\frac{F_i}{\langle X^2\rangle}\right)\left(\langle X^4\rangle - \frac{\langle X^2\rangle^2}{\langle 1\rangle}\right)d{\mit\Omega} = 3.055\times 10^{-3}.
\end{align}
