...
species-$s$ scalar
pressure, $\bpi_s$ is the species-$s$ viscosity tensor, ${\bf F}$ is the collisional friction force density, ${\bf q}_s$ is the species-$s$ heat flux density, $w_s$ is the species-$s$ collisional heating rate density, and ${\bf E}$ is the ambient electric field-strength. Moreover, $d_e/dt\equiv \partial/\partial t + {\bf V}_e\cdot\nabla$ in the electron fluid equations, (\ref{e3.79a})--(\ref{e3.79c}), whereas  $d_i/dt\equiv \partial/\partial t + {\bf V}_i\cdot\nabla$ in the ion fluid equations,  (\ref{e3.80a})--(\ref{e3.80c}). 
...
component ${\bf F}_T$ corresponding to the so-called {\em thermal force}. Thus,
\begin{equation}
...
of the fact that $\delta j_r\simeq 0$ in a radially thin current sheet. 
...
\section{Reduced Drift-MHD Resonant Plasma Response Model}
\subsection{Introduction}
...
\end{equation}
which automatically satisfies Equation~(\ref{e230}). (Here, we are assuming that $|r-r_s|\ll a$ inside the resonant layer.) Here, $\delta b(\hat{x},\zeta)$ and $\psi(\hat{x},\zeta)$ are assumed to be small
...
\end{equation}
where $\chi_\phi = \mu_{\perp}/(n_0\,m_i)$, $\hat{\chi}_\phi = \chi_\phi/(l\,V_A)$, and $\mu_\perp$ is the anomalous
...
Let us define $V=\hat{V}_{\parallel\,i}$ and $N=\delta p/c_\beta$. Equations~(\ref{e252})--(\ref{e254}), (\ref{e256}), (\ref{e258})--(\ref{e260}), (\ref{e262}), and (\ref{e263}) yield the following closed set of equations (Fitzpatrick \& Waelbroeck 2005):
...
If we neglect $d_\beta$ in our reduced drift-MHD model, which is equivalent to saying that the ion sound
...
ion sound radius is {\em not}\/ negligibly small compared to the radial extents of resonant layers in present-day
tokamak plasmas. Hence, the `single-fluid' reduced MHD model, (\ref{e275})--(\ref{e276}), provides  a very poor description
of the response of  resonant layers in tokamak plasmas. 
...
\section{Linear Resonant Plasma Response Model}
\subsection{Introduction}
The aim of this section is to employ the reduced, drift-MHD, resonant, plasma response model derived in the
