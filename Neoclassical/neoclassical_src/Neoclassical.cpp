...
void Neoclassical::Read_Profiles ()
{
  ...
  // Output profiles
  FILE* file = OpenFilew ((char*) "Stage1/profiles.txt");
  for (int j = 0; j < NPSI; j++)
...
void Neoclassical::Get_Derived ()
{
  ...
  FILE* file = OpenFilew ((char*) "Stage1/diamagnetic.txt");
  for (int j = 0; j < nres; j++)
    fprintf (file, "%16.9e %16.9e %16.9e %16.9e %16.9e %16.9e %16.9e\n",
	     rk (j), wEk (j) /1.e3, w_ast_ek (j) /1.e3, w_ast_ik (j) /1.e3, w_ast_Ik(j) /1.e3, Zeffk(j), NNk(j) /1.e19);
  fclose (file);
  ...
  // Output timescales
  file = OpenFilew ((char*) "Stage1/timescale.txt");
  for (int j = 0; j < nres; j++)
  ...
  file = OpenFilew ((char*) "Stage1/neoclassical.txt");
  for (int j = 0; j < nres; j++)
...
void Neoclassical::Get_Parameters ()
{
  ...
  // Output parameters
  FILE* file = OpenFilew ((char*) "Stage1/parameters.txt");
  for (int j = 0; j < nres; j++)
  ...
  for (int j = 0; j < nres; j++)
    {
      ...
      if (FREQ < 0)
	w_nonlinear (j) = w_linear (j);
      if (FREQ > 0)
	w_nonlinear (j) = w_EB     (j);

      w_fac (j) = (w_nonlinear (j)  - w_EB (j)) /(w_linear (j) - w_EB (j));
      
      printf ("m = %3d  w_linear = %11.4e  w_nonlinear = %11.4e  w_EB = %11.4e  fac = %11.4e\n",
	      mk(j), w_linear (j) /1.e3, w_nonlinear (j) /1.e3, w_EB (j) /1.e3, w_fac (j));
    }

  // ..................
  // Output frequencies
  // ..................
  FILE* file = OpenFilew ((char*) "Stage1/omega.txt");
  for (int j = 0; j < nres; j++)
    fprintf (file, "%16.9e %16.9e %16.9e %16.9e\n", rk (j), w_linear (j) /1.e3, w_nonlinear (j) /1.e3, w_EB (j) /1.e3);
  fclose (file);
}
...
{
  FILE* file = OpenFilew ((char*) "nFile");
