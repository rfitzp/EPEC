...
void Neoclassical::fFileInterp (vector<string> fFileName, vector<double> fFileTime, int fFileNumber, double time)
{
  if (fFileNumber < 2)
    {
      printf ("NEOCLASSICAL::fFileInterp - fFileNumber must be greater than unity\n");
      exit (1);
    }
					    ...
					    char* fFile4, double time4, char* fFile, double time)
{
  ...
  for (int j = 0; j < nres_4; j++)
    {
      if (fscanf (file, "%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
		  &mres_4(j), &u1_4(j), &u2_4(j), &u3_4(j), &u4_4(j), &u5_4(j), &u6_4(j), &u7_4(j), &u8_4(j), &u9_4(j), &u10_4(j)) != 11)
	{
